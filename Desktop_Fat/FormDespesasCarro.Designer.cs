namespace Dll_Forms_Fat
{
	partial class FormDespesasCarro
	{
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.TabCadastroVeiculos = new System.Windows.Forms.TabControl();
			this.tabAbastecimentos = new System.Windows.Forms.TabPage();
			this.groupBox3 = new System.Windows.Forms.GroupBox();
			this.button2 = new System.Windows.Forms.Button();
			this.label11 = new System.Windows.Forms.Label();
			this.label10 = new System.Windows.Forms.Label();
			this.dateTimePicker2 = new System.Windows.Forms.DateTimePicker();
			this.dateTimePicker1 = new System.Windows.Forms.DateTimePicker();
			this.groupBox2 = new System.Windows.Forms.GroupBox();
			this.dataAbastecimento = new System.Windows.Forms.DataGridView();
			this.groupAbastecimento = new System.Windows.Forms.GroupBox();
			this.dateAbastecimento = new System.Windows.Forms.DateTimePicker();
			this.btnAddAbastecimento = new System.Windows.Forms.Button();
			this.txtTotalAbastecimento = new System.Windows.Forms.TextBox();
			this.lblTotal = new System.Windows.Forms.Label();
			this.txtValorUnitAbastecimento = new System.Windows.Forms.TextBox();
			this.label6 = new System.Windows.Forms.Label();
			this.label9 = new System.Windows.Forms.Label();
			this.comboCombustivelAbastecimento = new System.Windows.Forms.ComboBox();
			this.txtQtdLitrosAbastecimento = new System.Windows.Forms.TextBox();
			this.label7 = new System.Windows.Forms.Label();
			this.label8 = new System.Windows.Forms.Label();
			this.tabManutencao = new System.Windows.Forms.TabPage();
			this.groupBox5 = new System.Windows.Forms.GroupBox();
			this.button4 = new System.Windows.Forms.Button();
			this.label13 = new System.Windows.Forms.Label();
			this.label17 = new System.Windows.Forms.Label();
			this.dateTimePicker3 = new System.Windows.Forms.DateTimePicker();
			this.dateTimePicker4 = new System.Windows.Forms.DateTimePicker();
			this.groupBox6 = new System.Windows.Forms.GroupBox();
			this.dataGridManutencao = new System.Windows.Forms.DataGridView();
			this.groupManutencao = new System.Windows.Forms.GroupBox();
			this.dateManutencao = new System.Windows.Forms.DateTimePicker();
			this.txtDescricaoManutencao = new System.Windows.Forms.RichTextBox();
			this.btnAddManutencao = new System.Windows.Forms.Button();
			this.label12 = new System.Windows.Forms.Label();
			this.txtKmManutencao = new System.Windows.Forms.TextBox();
			this.txtValorManutencao = new System.Windows.Forms.TextBox();
			this.label14 = new System.Windows.Forms.Label();
			this.label15 = new System.Windows.Forms.Label();
			this.label16 = new System.Windows.Forms.Label();
			this.tabSinistros = new System.Windows.Forms.TabPage();
			this.groupBox7 = new System.Windows.Forms.GroupBox();
			this.button5 = new System.Windows.Forms.Button();
			this.label18 = new System.Windows.Forms.Label();
			this.label19 = new System.Windows.Forms.Label();
			this.dateTimePicker5 = new System.Windows.Forms.DateTimePicker();
			this.dateTimePicker6 = new System.Windows.Forms.DateTimePicker();
			this.groupBox8 = new System.Windows.Forms.GroupBox();
			this.dataGridSinistros = new System.Windows.Forms.DataGridView();
			this.groupSinistros = new System.Windows.Forms.GroupBox();
			this.dateSinistros = new System.Windows.Forms.DateTimePicker();
			this.txtDescricaoSinistros = new System.Windows.Forms.RichTextBox();
			this.btnAddSinistros = new System.Windows.Forms.Button();
			this.label20 = new System.Windows.Forms.Label();
			this.txtKmSinistro = new System.Windows.Forms.TextBox();
			this.txtValorSinistro = new System.Windows.Forms.TextBox();
			this.label21 = new System.Windows.Forms.Label();
			this.label22 = new System.Windows.Forms.Label();
			this.label23 = new System.Windows.Forms.Label();
			this.tabObrigacoes = new System.Windows.Forms.TabPage();
			this.txtTipoObrigacao = new System.Windows.Forms.TextBox();
			this.dateObrigacoes = new System.Windows.Forms.DateTimePicker();
			this.lblValorTotalObrigacoes = new System.Windows.Forms.Label();
			this.label28 = new System.Windows.Forms.Label();
			this.groupBox10 = new System.Windows.Forms.GroupBox();
			this.dataGridObrigacoes = new System.Windows.Forms.DataGridView();
			this.label27 = new System.Windows.Forms.Label();
			this.btnAddObrigacoes = new System.Windows.Forms.Button();
			this.txtValorObrigacoes = new System.Windows.Forms.TextBox();
			this.label26 = new System.Windows.Forms.Label();
			this.txtDescricaoObrigacoes = new System.Windows.Forms.TextBox();
			this.label25 = new System.Windows.Forms.Label();
			this.label24 = new System.Windows.Forms.Label();
			this.tabMultas = new System.Windows.Forms.TabPage();
			this.dateMultaVencimento = new System.Windows.Forms.DateTimePicker();
			this.dateMulta = new System.Windows.Forms.DateTimePicker();
			this.label2 = new System.Windows.Forms.Label();
			this.dataGridMultas = new System.Windows.Forms.DataGridView();
			this.btnSalvarMulta = new System.Windows.Forms.Button();
			this.label4 = new System.Windows.Forms.Label();
			this.txtPontosMulta = new System.Windows.Forms.TextBox();
			this.label29 = new System.Windows.Forms.Label();
			this.txtValorMulta = new System.Windows.Forms.TextBox();
			this.txtDescricaoMulta = new System.Windows.Forms.TextBox();
			this.label30 = new System.Windows.Forms.Label();
			this.label31 = new System.Windows.Forms.Label();
			this.comboPlaca = new System.Windows.Forms.ComboBox();
			this.txtNomeCarro = new System.Windows.Forms.TextBox();
			this.label1 = new System.Windows.Forms.Label();
			this.TabCadastroVeiculos.SuspendLayout();
			this.tabAbastecimentos.SuspendLayout();
			this.groupBox3.SuspendLayout();
			this.groupBox2.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataAbastecimento)).BeginInit();
			this.groupAbastecimento.SuspendLayout();
			this.tabManutencao.SuspendLayout();
			this.groupBox5.SuspendLayout();
			this.groupBox6.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataGridManutencao)).BeginInit();
			this.groupManutencao.SuspendLayout();
			this.tabSinistros.SuspendLayout();
			this.groupBox7.SuspendLayout();
			this.groupBox8.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataGridSinistros)).BeginInit();
			this.groupSinistros.SuspendLayout();
			this.tabObrigacoes.SuspendLayout();
			this.groupBox10.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataGridObrigacoes)).BeginInit();
			this.tabMultas.SuspendLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataGridMultas)).BeginInit();
			this.SuspendLayout();
			// 
			// TabCadastroVeiculos
			// 
			this.TabCadastroVeiculos.Controls.Add(this.tabAbastecimentos);
			this.TabCadastroVeiculos.Controls.Add(this.tabManutencao);
			this.TabCadastroVeiculos.Controls.Add(this.tabSinistros);
			this.TabCadastroVeiculos.Controls.Add(this.tabObrigacoes);
			this.TabCadastroVeiculos.Controls.Add(this.tabMultas);
			this.TabCadastroVeiculos.Location = new System.Drawing.Point(12, 67);
			this.TabCadastroVeiculos.Name = "TabCadastroVeiculos";
			this.TabCadastroVeiculos.SelectedIndex = 0;
			this.TabCadastroVeiculos.Size = new System.Drawing.Size(760, 380);
			this.TabCadastroVeiculos.TabIndex = 18;
			// 
			// tabAbastecimentos
			// 
			this.tabAbastecimentos.BackColor = System.Drawing.Color.WhiteSmoke;
			this.tabAbastecimentos.Controls.Add(this.groupBox3);
			this.tabAbastecimentos.Controls.Add(this.groupBox2);
			this.tabAbastecimentos.Controls.Add(this.groupAbastecimento);
			this.tabAbastecimentos.Location = new System.Drawing.Point(4, 22);
			this.tabAbastecimentos.Name = "tabAbastecimentos";
			this.tabAbastecimentos.Padding = new System.Windows.Forms.Padding(3);
			this.tabAbastecimentos.Size = new System.Drawing.Size(752, 354);
			this.tabAbastecimentos.TabIndex = 3;
			this.tabAbastecimentos.Text = "Abastecimentos";
			// 
			// groupBox3
			// 
			this.groupBox3.Controls.Add(this.button2);
			this.groupBox3.Controls.Add(this.label11);
			this.groupBox3.Controls.Add(this.label10);
			this.groupBox3.Controls.Add(this.dateTimePicker2);
			this.groupBox3.Controls.Add(this.dateTimePicker1);
			this.groupBox3.Location = new System.Drawing.Point(7, 250);
			this.groupBox3.Name = "groupBox3";
			this.groupBox3.Size = new System.Drawing.Size(738, 79);
			this.groupBox3.TabIndex = 14;
			this.groupBox3.TabStop = false;
			this.groupBox3.Text = "Consulta por Período:";
			// 
			// button2
			// 
			this.button2.Location = new System.Drawing.Point(541, 34);
			this.button2.Name = "button2";
			this.button2.Size = new System.Drawing.Size(69, 20);
			this.button2.TabIndex = 13;
			this.button2.Text = "Consultar...";
			this.button2.UseVisualStyleBackColor = true;
			// 
			// label11
			// 
			this.label11.AutoSize = true;
			this.label11.Location = new System.Drawing.Point(274, 41);
			this.label11.Name = "label11";
			this.label11.Size = new System.Drawing.Size(13, 13);
			this.label11.TabIndex = 3;
			this.label11.Text = "a";
			// 
			// label10
			// 
			this.label10.AutoSize = true;
			this.label10.Location = new System.Drawing.Point(11, 41);
			this.label10.Name = "label10";
			this.label10.Size = new System.Drawing.Size(24, 13);
			this.label10.TabIndex = 2;
			this.label10.Text = "De:";
			// 
			// dateTimePicker2
			// 
			this.dateTimePicker2.Location = new System.Drawing.Point(293, 34);
			this.dateTimePicker2.Name = "dateTimePicker2";
			this.dateTimePicker2.Size = new System.Drawing.Size(229, 20);
			this.dateTimePicker2.TabIndex = 1;
			// 
			// dateTimePicker1
			// 
			this.dateTimePicker1.Location = new System.Drawing.Point(41, 35);
			this.dateTimePicker1.Name = "dateTimePicker1";
			this.dateTimePicker1.Size = new System.Drawing.Size(227, 20);
			this.dateTimePicker1.TabIndex = 0;
			// 
			// groupBox2
			// 
			this.groupBox2.Controls.Add(this.dataAbastecimento);
			this.groupBox2.Location = new System.Drawing.Point(6, 97);
			this.groupBox2.Name = "groupBox2";
			this.groupBox2.Size = new System.Drawing.Size(739, 146);
			this.groupBox2.TabIndex = 13;
			this.groupBox2.TabStop = false;
			this.groupBox2.Text = "Histórico de Consumo:";
			// 
			// dataAbastecimento
			// 
			this.dataAbastecimento.AllowUserToAddRows = false;
			this.dataAbastecimento.AllowUserToDeleteRows = false;
			this.dataAbastecimento.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize;
			this.dataAbastecimento.Location = new System.Drawing.Point(5, 20);
			this.dataAbastecimento.Name = "dataAbastecimento";
			this.dataAbastecimento.ReadOnly = true;
			this.dataAbastecimento.Size = new System.Drawing.Size(719, 120);
			this.dataAbastecimento.TabIndex = 0;
			// 
			// groupAbastecimento
			// 
			this.groupAbastecimento.Controls.Add(this.dateAbastecimento);
			this.groupAbastecimento.Controls.Add(this.btnAddAbastecimento);
			this.groupAbastecimento.Controls.Add(this.txtTotalAbastecimento);
			this.groupAbastecimento.Controls.Add(this.lblTotal);
			this.groupAbastecimento.Controls.Add(this.txtValorUnitAbastecimento);
			this.groupAbastecimento.Controls.Add(this.label6);
			this.groupAbastecimento.Controls.Add(this.label9);
			this.groupAbastecimento.Controls.Add(this.comboCombustivelAbastecimento);
			this.groupAbastecimento.Controls.Add(this.txtQtdLitrosAbastecimento);
			this.groupAbastecimento.Controls.Add(this.label7);
			this.groupAbastecimento.Controls.Add(this.label8);
			this.groupAbastecimento.Location = new System.Drawing.Point(6, 6);
			this.groupAbastecimento.Name = "groupAbastecimento";
			this.groupAbastecimento.Size = new System.Drawing.Size(740, 85);
			this.groupAbastecimento.TabIndex = 12;
			this.groupAbastecimento.TabStop = false;
			this.groupAbastecimento.Text = "Lançamento de Consumo";
			// 
			// dateAbastecimento
			// 
			this.dateAbastecimento.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateAbastecimento.Location = new System.Drawing.Point(6, 40);
			this.dateAbastecimento.Name = "dateAbastecimento";
			this.dateAbastecimento.Size = new System.Drawing.Size(101, 20);
			this.dateAbastecimento.TabIndex = 13;
			// 
			// btnAddAbastecimento
			// 
			this.btnAddAbastecimento.Location = new System.Drawing.Point(618, 40);
			this.btnAddAbastecimento.Name = "btnAddAbastecimento";
			this.btnAddAbastecimento.Size = new System.Drawing.Size(23, 20);
			this.btnAddAbastecimento.TabIndex = 12;
			this.btnAddAbastecimento.Text = "+";
			this.btnAddAbastecimento.UseVisualStyleBackColor = true;
			this.btnAddAbastecimento.Click += new System.EventHandler(this.BtnAddAbastecimento_Click);
			// 
			// txtTotalAbastecimento
			// 
			this.txtTotalAbastecimento.Location = new System.Drawing.Point(511, 40);
			this.txtTotalAbastecimento.Name = "txtTotalAbastecimento";
			this.txtTotalAbastecimento.ReadOnly = true;
			this.txtTotalAbastecimento.Size = new System.Drawing.Size(100, 20);
			this.txtTotalAbastecimento.TabIndex = 11;
			// 
			// lblTotal
			// 
			this.lblTotal.AutoSize = true;
			this.lblTotal.Location = new System.Drawing.Point(508, 24);
			this.lblTotal.Name = "lblTotal";
			this.lblTotal.Size = new System.Drawing.Size(31, 13);
			this.lblTotal.TabIndex = 10;
			this.lblTotal.Text = "Total";
			// 
			// txtValorUnitAbastecimento
			// 
			this.txtValorUnitAbastecimento.Location = new System.Drawing.Point(381, 41);
			this.txtValorUnitAbastecimento.MaxLength = 6;
			this.txtValorUnitAbastecimento.Name = "txtValorUnitAbastecimento";
			this.txtValorUnitAbastecimento.Size = new System.Drawing.Size(100, 20);
			this.txtValorUnitAbastecimento.TabIndex = 9;
			this.txtValorUnitAbastecimento.TextChanged += new System.EventHandler(this.txtValorUnitAbastecimento_TextChanged);
			this.txtValorUnitAbastecimento.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtValorUnitAbastecimento_KeyPress);
			// 
			// label6
			// 
			this.label6.AutoSize = true;
			this.label6.Location = new System.Drawing.Point(3, 24);
			this.label6.Name = "label6";
			this.label6.Size = new System.Drawing.Size(33, 13);
			this.label6.TabIndex = 3;
			this.label6.Text = "Data:";
			// 
			// label9
			// 
			this.label9.AutoSize = true;
			this.label9.Location = new System.Drawing.Point(378, 25);
			this.label9.Name = "label9";
			this.label9.Size = new System.Drawing.Size(73, 13);
			this.label9.TabIndex = 8;
			this.label9.Text = "Valor Unitário:";
			// 
			// comboCombustivelAbastecimento
			// 
			this.comboCombustivelAbastecimento.AutoCompleteCustomSource.AddRange(new string[] {
            "Etanol",
            "Gasolina",
            "GNV",
            "Diesel"});
			this.comboCombustivelAbastecimento.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			this.comboCombustivelAbastecimento.FormattingEnabled = true;
			this.comboCombustivelAbastecimento.Items.AddRange(new object[] {
            "GASOLINA",
            "ETANOL",
            "DIESEL"});
			this.comboCombustivelAbastecimento.Location = new System.Drawing.Point(148, 40);
			this.comboCombustivelAbastecimento.Name = "comboCombustivelAbastecimento";
			this.comboCombustivelAbastecimento.Size = new System.Drawing.Size(121, 21);
			this.comboCombustivelAbastecimento.TabIndex = 4;
			// 
			// txtQtdLitrosAbastecimento
			// 
			this.txtQtdLitrosAbastecimento.Location = new System.Drawing.Point(275, 41);
			this.txtQtdLitrosAbastecimento.MaxLength = 3;
			this.txtQtdLitrosAbastecimento.Name = "txtQtdLitrosAbastecimento";
			this.txtQtdLitrosAbastecimento.Size = new System.Drawing.Size(100, 20);
			this.txtQtdLitrosAbastecimento.TabIndex = 7;
			this.txtQtdLitrosAbastecimento.TextChanged += new System.EventHandler(this.txtQtdLitrosAbastecimento_TextChanged);
			this.txtQtdLitrosAbastecimento.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtQtdLitrosAbastecimento_KeyPress);
			// 
			// label7
			// 
			this.label7.AutoSize = true;
			this.label7.Location = new System.Drawing.Point(145, 24);
			this.label7.Name = "label7";
			this.label7.Size = new System.Drawing.Size(64, 13);
			this.label7.TabIndex = 5;
			this.label7.Text = "Combustivel";
			// 
			// label8
			// 
			this.label8.AutoSize = true;
			this.label8.Location = new System.Drawing.Point(272, 25);
			this.label8.Name = "label8";
			this.label8.Size = new System.Drawing.Size(55, 13);
			this.label8.TabIndex = 6;
			this.label8.Text = "Qtd Litros:";
			// 
			// tabManutencao
			// 
			this.tabManutencao.BackColor = System.Drawing.Color.WhiteSmoke;
			this.tabManutencao.Controls.Add(this.groupBox5);
			this.tabManutencao.Controls.Add(this.groupBox6);
			this.tabManutencao.Controls.Add(this.groupManutencao);
			this.tabManutencao.Location = new System.Drawing.Point(4, 22);
			this.tabManutencao.Name = "tabManutencao";
			this.tabManutencao.Padding = new System.Windows.Forms.Padding(3);
			this.tabManutencao.Size = new System.Drawing.Size(752, 354);
			this.tabManutencao.TabIndex = 1;
			this.tabManutencao.Text = "Manutenções";
			// 
			// groupBox5
			// 
			this.groupBox5.Controls.Add(this.button4);
			this.groupBox5.Controls.Add(this.label13);
			this.groupBox5.Controls.Add(this.label17);
			this.groupBox5.Controls.Add(this.dateTimePicker3);
			this.groupBox5.Controls.Add(this.dateTimePicker4);
			this.groupBox5.Location = new System.Drawing.Point(8, 262);
			this.groupBox5.Name = "groupBox5";
			this.groupBox5.Size = new System.Drawing.Size(738, 79);
			this.groupBox5.TabIndex = 16;
			this.groupBox5.TabStop = false;
			this.groupBox5.Text = "Consulta por Período:";
			// 
			// button4
			// 
			this.button4.Location = new System.Drawing.Point(541, 34);
			this.button4.Name = "button4";
			this.button4.Size = new System.Drawing.Size(69, 20);
			this.button4.TabIndex = 13;
			this.button4.Text = "Consultar...";
			this.button4.UseVisualStyleBackColor = true;
			// 
			// label13
			// 
			this.label13.AutoSize = true;
			this.label13.Location = new System.Drawing.Point(274, 41);
			this.label13.Name = "label13";
			this.label13.Size = new System.Drawing.Size(13, 13);
			this.label13.TabIndex = 3;
			this.label13.Text = "a";
			// 
			// label17
			// 
			this.label17.AutoSize = true;
			this.label17.Location = new System.Drawing.Point(11, 41);
			this.label17.Name = "label17";
			this.label17.Size = new System.Drawing.Size(24, 13);
			this.label17.TabIndex = 2;
			this.label17.Text = "De:";
			// 
			// dateTimePicker3
			// 
			this.dateTimePicker3.Location = new System.Drawing.Point(293, 34);
			this.dateTimePicker3.Name = "dateTimePicker3";
			this.dateTimePicker3.Size = new System.Drawing.Size(229, 20);
			this.dateTimePicker3.TabIndex = 1;
			// 
			// dateTimePicker4
			// 
			this.dateTimePicker4.Location = new System.Drawing.Point(41, 35);
			this.dateTimePicker4.Name = "dateTimePicker4";
			this.dateTimePicker4.Size = new System.Drawing.Size(227, 20);
			this.dateTimePicker4.TabIndex = 0;
			// 
			// groupBox6
			// 
			this.groupBox6.Controls.Add(this.dataGridManutencao);
			this.groupBox6.Location = new System.Drawing.Point(7, 109);
			this.groupBox6.Name = "groupBox6";
			this.groupBox6.Size = new System.Drawing.Size(739, 146);
			this.groupBox6.TabIndex = 15;
			this.groupBox6.TabStop = false;
			this.groupBox6.Text = "Histórico de Manutenções:";
			// 
			// dataGridManutencao
			// 
			this.dataGridManutencao.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize;
			this.dataGridManutencao.Location = new System.Drawing.Point(5, 20);
			this.dataGridManutencao.Name = "dataGridManutencao";
			this.dataGridManutencao.Size = new System.Drawing.Size(719, 120);
			this.dataGridManutencao.TabIndex = 0;
			// 
			// groupManutencao
			// 
			this.groupManutencao.Controls.Add(this.dateManutencao);
			this.groupManutencao.Controls.Add(this.txtDescricaoManutencao);
			this.groupManutencao.Controls.Add(this.btnAddManutencao);
			this.groupManutencao.Controls.Add(this.label12);
			this.groupManutencao.Controls.Add(this.txtKmManutencao);
			this.groupManutencao.Controls.Add(this.txtValorManutencao);
			this.groupManutencao.Controls.Add(this.label14);
			this.groupManutencao.Controls.Add(this.label15);
			this.groupManutencao.Controls.Add(this.label16);
			this.groupManutencao.Location = new System.Drawing.Point(6, 6);
			this.groupManutencao.Name = "groupManutencao";
			this.groupManutencao.Size = new System.Drawing.Size(740, 97);
			this.groupManutencao.TabIndex = 13;
			this.groupManutencao.TabStop = false;
			this.groupManutencao.Text = "Lançamento de manutenção";
			// 
			// dateManutencao
			// 
			this.dateManutencao.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateManutencao.Location = new System.Drawing.Point(1, 40);
			this.dateManutencao.Name = "dateManutencao";
			this.dateManutencao.Size = new System.Drawing.Size(101, 20);
			this.dateManutencao.TabIndex = 14;
			// 
			// txtDescricaoManutencao
			// 
			this.txtDescricaoManutencao.Location = new System.Drawing.Point(219, 40);
			this.txtDescricaoManutencao.Name = "txtDescricaoManutencao";
			this.txtDescricaoManutencao.Size = new System.Drawing.Size(361, 46);
			this.txtDescricaoManutencao.TabIndex = 13;
			this.txtDescricaoManutencao.Text = "";
			// 
			// btnAddManutencao
			// 
			this.btnAddManutencao.Location = new System.Drawing.Point(709, 40);
			this.btnAddManutencao.Name = "btnAddManutencao";
			this.btnAddManutencao.Size = new System.Drawing.Size(23, 20);
			this.btnAddManutencao.TabIndex = 12;
			this.btnAddManutencao.Text = "+";
			this.btnAddManutencao.UseVisualStyleBackColor = true;
			this.btnAddManutencao.Click += new System.EventHandler(this.BtnAddManutencao_Click);
			// 
			// label12
			// 
			this.label12.AutoSize = true;
			this.label12.Location = new System.Drawing.Point(110, 24);
			this.label12.Name = "label12";
			this.label12.Size = new System.Drawing.Size(99, 13);
			this.label12.TabIndex = 0;
			this.label12.Text = "Km na manutenção";
			// 
			// txtKmManutencao
			// 
			this.txtKmManutencao.Location = new System.Drawing.Point(113, 40);
			this.txtKmManutencao.MaxLength = 7;
			this.txtKmManutencao.Name = "txtKmManutencao";
			this.txtKmManutencao.Size = new System.Drawing.Size(100, 20);
			this.txtKmManutencao.TabIndex = 1;
			this.txtKmManutencao.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtKmManutencao_KeyPress);
			// 
			// txtValorManutencao
			// 
			this.txtValorManutencao.Location = new System.Drawing.Point(599, 40);
			this.txtValorManutencao.MaxLength = 6;
			this.txtValorManutencao.Name = "txtValorManutencao";
			this.txtValorManutencao.Size = new System.Drawing.Size(100, 20);
			this.txtValorManutencao.TabIndex = 9;
			this.txtValorManutencao.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtValorManutencao_KeyPress);
			// 
			// label14
			// 
			this.label14.AutoSize = true;
			this.label14.Location = new System.Drawing.Point(3, 24);
			this.label14.Name = "label14";
			this.label14.Size = new System.Drawing.Size(33, 13);
			this.label14.TabIndex = 3;
			this.label14.Text = "Data:";
			// 
			// label15
			// 
			this.label15.AutoSize = true;
			this.label15.Location = new System.Drawing.Point(596, 23);
			this.label15.Name = "label15";
			this.label15.Size = new System.Drawing.Size(34, 13);
			this.label15.TabIndex = 8;
			this.label15.Text = "Valor:";
			// 
			// label16
			// 
			this.label16.AutoSize = true;
			this.label16.Location = new System.Drawing.Point(216, 24);
			this.label16.Name = "label16";
			this.label16.Size = new System.Drawing.Size(55, 13);
			this.label16.TabIndex = 5;
			this.label16.Text = "Descrição";
			// 
			// tabSinistros
			// 
			this.tabSinistros.BackColor = System.Drawing.Color.WhiteSmoke;
			this.tabSinistros.Controls.Add(this.groupBox7);
			this.tabSinistros.Controls.Add(this.groupBox8);
			this.tabSinistros.Controls.Add(this.groupSinistros);
			this.tabSinistros.Location = new System.Drawing.Point(4, 22);
			this.tabSinistros.Name = "tabSinistros";
			this.tabSinistros.Size = new System.Drawing.Size(752, 354);
			this.tabSinistros.TabIndex = 2;
			this.tabSinistros.Text = "Sinistros";
			// 
			// groupBox7
			// 
			this.groupBox7.Controls.Add(this.button5);
			this.groupBox7.Controls.Add(this.label18);
			this.groupBox7.Controls.Add(this.label19);
			this.groupBox7.Controls.Add(this.dateTimePicker5);
			this.groupBox7.Controls.Add(this.dateTimePicker6);
			this.groupBox7.Location = new System.Drawing.Point(8, 266);
			this.groupBox7.Name = "groupBox7";
			this.groupBox7.Size = new System.Drawing.Size(738, 79);
			this.groupBox7.TabIndex = 19;
			this.groupBox7.TabStop = false;
			this.groupBox7.Text = "Consulta por Período:";
			// 
			// button5
			// 
			this.button5.Location = new System.Drawing.Point(541, 34);
			this.button5.Name = "button5";
			this.button5.Size = new System.Drawing.Size(69, 20);
			this.button5.TabIndex = 13;
			this.button5.Text = "Consultar...";
			this.button5.UseVisualStyleBackColor = true;
			// 
			// label18
			// 
			this.label18.AutoSize = true;
			this.label18.Location = new System.Drawing.Point(274, 41);
			this.label18.Name = "label18";
			this.label18.Size = new System.Drawing.Size(13, 13);
			this.label18.TabIndex = 3;
			this.label18.Text = "a";
			// 
			// label19
			// 
			this.label19.AutoSize = true;
			this.label19.Location = new System.Drawing.Point(11, 41);
			this.label19.Name = "label19";
			this.label19.Size = new System.Drawing.Size(24, 13);
			this.label19.TabIndex = 2;
			this.label19.Text = "De:";
			// 
			// dateTimePicker5
			// 
			this.dateTimePicker5.Location = new System.Drawing.Point(293, 34);
			this.dateTimePicker5.Name = "dateTimePicker5";
			this.dateTimePicker5.Size = new System.Drawing.Size(229, 20);
			this.dateTimePicker5.TabIndex = 1;
			// 
			// dateTimePicker6
			// 
			this.dateTimePicker6.Location = new System.Drawing.Point(41, 35);
			this.dateTimePicker6.Name = "dateTimePicker6";
			this.dateTimePicker6.Size = new System.Drawing.Size(227, 20);
			this.dateTimePicker6.TabIndex = 0;
			// 
			// groupBox8
			// 
			this.groupBox8.Controls.Add(this.dataGridSinistros);
			this.groupBox8.Location = new System.Drawing.Point(7, 113);
			this.groupBox8.Name = "groupBox8";
			this.groupBox8.Size = new System.Drawing.Size(739, 146);
			this.groupBox8.TabIndex = 18;
			this.groupBox8.TabStop = false;
			this.groupBox8.Text = "Histórico de Sinistros:";
			// 
			// dataGridSinistros
			// 
			this.dataGridSinistros.AllowUserToAddRows = false;
			this.dataGridSinistros.AllowUserToDeleteRows = false;
			this.dataGridSinistros.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize;
			this.dataGridSinistros.Location = new System.Drawing.Point(5, 20);
			this.dataGridSinistros.Name = "dataGridSinistros";
			this.dataGridSinistros.ReadOnly = true;
			this.dataGridSinistros.Size = new System.Drawing.Size(719, 120);
			this.dataGridSinistros.TabIndex = 0;
			// 
			// groupSinistros
			// 
			this.groupSinistros.Controls.Add(this.dateSinistros);
			this.groupSinistros.Controls.Add(this.txtDescricaoSinistros);
			this.groupSinistros.Controls.Add(this.btnAddSinistros);
			this.groupSinistros.Controls.Add(this.label20);
			this.groupSinistros.Controls.Add(this.txtKmSinistro);
			this.groupSinistros.Controls.Add(this.txtValorSinistro);
			this.groupSinistros.Controls.Add(this.label21);
			this.groupSinistros.Controls.Add(this.label22);
			this.groupSinistros.Controls.Add(this.label23);
			this.groupSinistros.Location = new System.Drawing.Point(6, 10);
			this.groupSinistros.Name = "groupSinistros";
			this.groupSinistros.Size = new System.Drawing.Size(740, 97);
			this.groupSinistros.TabIndex = 17;
			this.groupSinistros.TabStop = false;
			this.groupSinistros.Text = "Lançamento de sinistros";
			// 
			// dateSinistros
			// 
			this.dateSinistros.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateSinistros.Location = new System.Drawing.Point(2, 40);
			this.dateSinistros.Name = "dateSinistros";
			this.dateSinistros.Size = new System.Drawing.Size(101, 20);
			this.dateSinistros.TabIndex = 14;
			// 
			// txtDescricaoSinistros
			// 
			this.txtDescricaoSinistros.Location = new System.Drawing.Point(219, 40);
			this.txtDescricaoSinistros.Name = "txtDescricaoSinistros";
			this.txtDescricaoSinistros.Size = new System.Drawing.Size(361, 46);
			this.txtDescricaoSinistros.TabIndex = 13;
			this.txtDescricaoSinistros.Text = "";
			// 
			// btnAddSinistros
			// 
			this.btnAddSinistros.Location = new System.Drawing.Point(709, 40);
			this.btnAddSinistros.Name = "btnAddSinistros";
			this.btnAddSinistros.Size = new System.Drawing.Size(23, 20);
			this.btnAddSinistros.TabIndex = 12;
			this.btnAddSinistros.Text = "+";
			this.btnAddSinistros.UseVisualStyleBackColor = true;
			this.btnAddSinistros.Click += new System.EventHandler(this.BtnAddSinistros_Click);
			// 
			// label20
			// 
			this.label20.AutoSize = true;
			this.label20.Location = new System.Drawing.Point(110, 24);
			this.label20.Name = "label20";
			this.label20.Size = new System.Drawing.Size(74, 13);
			this.label20.TabIndex = 0;
			this.label20.Text = "Km no Sinistro";
			// 
			// txtKmSinistro
			// 
			this.txtKmSinistro.Location = new System.Drawing.Point(113, 40);
			this.txtKmSinistro.MaxLength = 7;
			this.txtKmSinistro.Name = "txtKmSinistro";
			this.txtKmSinistro.Size = new System.Drawing.Size(100, 20);
			this.txtKmSinistro.TabIndex = 1;
			this.txtKmSinistro.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtKmSinistro_KeyPress);
			// 
			// txtValorSinistro
			// 
			this.txtValorSinistro.Location = new System.Drawing.Point(599, 40);
			this.txtValorSinistro.MaxLength = 7;
			this.txtValorSinistro.Name = "txtValorSinistro";
			this.txtValorSinistro.Size = new System.Drawing.Size(100, 20);
			this.txtValorSinistro.TabIndex = 9;
			this.txtValorSinistro.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtValorSinistro_KeyPress);
			// 
			// label21
			// 
			this.label21.AutoSize = true;
			this.label21.Location = new System.Drawing.Point(3, 24);
			this.label21.Name = "label21";
			this.label21.Size = new System.Drawing.Size(33, 13);
			this.label21.TabIndex = 3;
			this.label21.Text = "Data:";
			// 
			// label22
			// 
			this.label22.AutoSize = true;
			this.label22.Location = new System.Drawing.Point(596, 23);
			this.label22.Name = "label22";
			this.label22.Size = new System.Drawing.Size(34, 13);
			this.label22.TabIndex = 8;
			this.label22.Text = "Valor:";
			// 
			// label23
			// 
			this.label23.AutoSize = true;
			this.label23.Location = new System.Drawing.Point(216, 24);
			this.label23.Name = "label23";
			this.label23.Size = new System.Drawing.Size(55, 13);
			this.label23.TabIndex = 5;
			this.label23.Text = "Descrição";
			// 
			// tabObrigacoes
			// 
			this.tabObrigacoes.BackColor = System.Drawing.Color.WhiteSmoke;
			this.tabObrigacoes.Controls.Add(this.txtTipoObrigacao);
			this.tabObrigacoes.Controls.Add(this.dateObrigacoes);
			this.tabObrigacoes.Controls.Add(this.lblValorTotalObrigacoes);
			this.tabObrigacoes.Controls.Add(this.label28);
			this.tabObrigacoes.Controls.Add(this.groupBox10);
			this.tabObrigacoes.Controls.Add(this.label27);
			this.tabObrigacoes.Controls.Add(this.btnAddObrigacoes);
			this.tabObrigacoes.Controls.Add(this.txtValorObrigacoes);
			this.tabObrigacoes.Controls.Add(this.label26);
			this.tabObrigacoes.Controls.Add(this.txtDescricaoObrigacoes);
			this.tabObrigacoes.Controls.Add(this.label25);
			this.tabObrigacoes.Controls.Add(this.label24);
			this.tabObrigacoes.Location = new System.Drawing.Point(4, 22);
			this.tabObrigacoes.Name = "tabObrigacoes";
			this.tabObrigacoes.Size = new System.Drawing.Size(752, 354);
			this.tabObrigacoes.TabIndex = 4;
			this.tabObrigacoes.Text = "Obrigações Financeiras";
			// 
			// txtTipoObrigacao
			// 
			this.txtTipoObrigacao.Location = new System.Drawing.Point(129, 26);
			this.txtTipoObrigacao.Name = "txtTipoObrigacao";
			this.txtTipoObrigacao.Size = new System.Drawing.Size(372, 20);
			this.txtTipoObrigacao.TabIndex = 15;
			// 
			// dateObrigacoes
			// 
			this.dateObrigacoes.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateObrigacoes.Location = new System.Drawing.Point(15, 25);
			this.dateObrigacoes.Name = "dateObrigacoes";
			this.dateObrigacoes.Size = new System.Drawing.Size(97, 20);
			this.dateObrigacoes.TabIndex = 14;
			// 
			// lblValorTotalObrigacoes
			// 
			this.lblValorTotalObrigacoes.AutoSize = true;
			this.lblValorTotalObrigacoes.Font = new System.Drawing.Font("Microsoft Sans Serif", 11.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
			this.lblValorTotalObrigacoes.Location = new System.Drawing.Point(618, 305);
			this.lblValorTotalObrigacoes.Name = "lblValorTotalObrigacoes";
			this.lblValorTotalObrigacoes.Size = new System.Drawing.Size(59, 18);
			this.lblValorTotalObrigacoes.TabIndex = 12;
			this.lblValorTotalObrigacoes.Text = "R$ 0,00";
			// 
			// label28
			// 
			this.label28.AutoSize = true;
			this.label28.Location = new System.Drawing.Point(474, 309);
			this.label28.Name = "label28";
			this.label28.Size = new System.Drawing.Size(138, 13);
			this.label28.TabIndex = 11;
			this.label28.Text = "Valor Total das Obrigações:";
			// 
			// groupBox10
			// 
			this.groupBox10.Controls.Add(this.dataGridObrigacoes);
			this.groupBox10.Location = new System.Drawing.Point(13, 107);
			this.groupBox10.Name = "groupBox10";
			this.groupBox10.Size = new System.Drawing.Size(713, 189);
			this.groupBox10.TabIndex = 10;
			this.groupBox10.TabStop = false;
			this.groupBox10.Text = "Obrigações Registradas:";
			// 
			// dataGridObrigacoes
			// 
			this.dataGridObrigacoes.AllowUserToAddRows = false;
			this.dataGridObrigacoes.AllowUserToDeleteRows = false;
			this.dataGridObrigacoes.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize;
			this.dataGridObrigacoes.Location = new System.Drawing.Point(6, 19);
			this.dataGridObrigacoes.Name = "dataGridObrigacoes";
			this.dataGridObrigacoes.ReadOnly = true;
			this.dataGridObrigacoes.Size = new System.Drawing.Size(701, 150);
			this.dataGridObrigacoes.TabIndex = 0;
			// 
			// label27
			// 
			this.label27.AutoSize = true;
			this.label27.Location = new System.Drawing.Point(12, 56);
			this.label27.Name = "label27";
			this.label27.Size = new System.Drawing.Size(58, 13);
			this.label27.TabIndex = 9;
			this.label27.Text = "Descrição:";
			// 
			// btnAddObrigacoes
			// 
			this.btnAddObrigacoes.Location = new System.Drawing.Point(645, 28);
			this.btnAddObrigacoes.Name = "btnAddObrigacoes";
			this.btnAddObrigacoes.Size = new System.Drawing.Size(75, 21);
			this.btnAddObrigacoes.TabIndex = 6;
			this.btnAddObrigacoes.Text = "Adicionar";
			this.btnAddObrigacoes.UseVisualStyleBackColor = true;
			this.btnAddObrigacoes.Click += new System.EventHandler(this.BtnAddObrigacoes_Click);
			// 
			// txtValorObrigacoes
			// 
			this.txtValorObrigacoes.Location = new System.Drawing.Point(552, 28);
			this.txtValorObrigacoes.MaxLength = 7;
			this.txtValorObrigacoes.Name = "txtValorObrigacoes";
			this.txtValorObrigacoes.Size = new System.Drawing.Size(87, 20);
			this.txtValorObrigacoes.TabIndex = 5;
			this.txtValorObrigacoes.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtValorObrigacoes_KeyPress);
			// 
			// label26
			// 
			this.label26.AutoSize = true;
			this.label26.Location = new System.Drawing.Point(549, 10);
			this.label26.Name = "label26";
			this.label26.Size = new System.Drawing.Size(34, 13);
			this.label26.TabIndex = 4;
			this.label26.Text = "Valor:";
			// 
			// txtDescricaoObrigacoes
			// 
			this.txtDescricaoObrigacoes.Location = new System.Drawing.Point(13, 72);
			this.txtDescricaoObrigacoes.Name = "txtDescricaoObrigacoes";
			this.txtDescricaoObrigacoes.Size = new System.Drawing.Size(470, 20);
			this.txtDescricaoObrigacoes.TabIndex = 3;
			// 
			// label25
			// 
			this.label25.AutoSize = true;
			this.label25.Location = new System.Drawing.Point(126, 10);
			this.label25.Name = "label25";
			this.label25.Size = new System.Drawing.Size(31, 13);
			this.label25.TabIndex = 2;
			this.label25.Text = "Tipo:";
			// 
			// label24
			// 
			this.label24.AutoSize = true;
			this.label24.Location = new System.Drawing.Point(12, 10);
			this.label24.Name = "label24";
			this.label24.Size = new System.Drawing.Size(33, 13);
			this.label24.TabIndex = 0;
			this.label24.Text = "Data:";
			// 
			// tabMultas
			// 
			this.tabMultas.BackColor = System.Drawing.Color.WhiteSmoke;
			this.tabMultas.Controls.Add(this.dateMultaVencimento);
			this.tabMultas.Controls.Add(this.dateMulta);
			this.tabMultas.Controls.Add(this.label2);
			this.tabMultas.Controls.Add(this.dataGridMultas);
			this.tabMultas.Controls.Add(this.btnSalvarMulta);
			this.tabMultas.Controls.Add(this.label4);
			this.tabMultas.Controls.Add(this.txtPontosMulta);
			this.tabMultas.Controls.Add(this.label29);
			this.tabMultas.Controls.Add(this.txtValorMulta);
			this.tabMultas.Controls.Add(this.txtDescricaoMulta);
			this.tabMultas.Controls.Add(this.label30);
			this.tabMultas.Controls.Add(this.label31);
			this.tabMultas.Location = new System.Drawing.Point(4, 22);
			this.tabMultas.Name = "tabMultas";
			this.tabMultas.Padding = new System.Windows.Forms.Padding(3);
			this.tabMultas.Size = new System.Drawing.Size(752, 354);
			this.tabMultas.TabIndex = 5;
			this.tabMultas.Text = "Multas";
			// 
			// dateMultaVencimento
			// 
			this.dateMultaVencimento.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateMultaVencimento.Location = new System.Drawing.Point(13, 65);
			this.dateMultaVencimento.Name = "dateMultaVencimento";
			this.dateMultaVencimento.Size = new System.Drawing.Size(103, 20);
			this.dateMultaVencimento.TabIndex = 28;
			// 
			// dateMulta
			// 
			this.dateMulta.Format = System.Windows.Forms.DateTimePickerFormat.Short;
			this.dateMulta.Location = new System.Drawing.Point(13, 25);
			this.dateMulta.Name = "dateMulta";
			this.dateMulta.Size = new System.Drawing.Size(103, 20);
			this.dateMulta.TabIndex = 27;
			// 
			// label2
			// 
			this.label2.AutoSize = true;
			this.label2.Location = new System.Drawing.Point(10, 48);
			this.label2.Name = "label2";
			this.label2.Size = new System.Drawing.Size(106, 13);
			this.label2.TabIndex = 26;
			this.label2.Text = "Data de vencimento:";
			// 
			// dataGridMultas
			// 
			this.dataGridMultas.ColumnHeadersHeightSizeMode = System.Windows.Forms.DataGridViewColumnHeadersHeightSizeMode.AutoSize;
			this.dataGridMultas.Location = new System.Drawing.Point(4, 90);
			this.dataGridMultas.Name = "dataGridMultas";
			this.dataGridMultas.Size = new System.Drawing.Size(744, 271);
			this.dataGridMultas.TabIndex = 22;
			// 
			// btnSalvarMulta
			// 
			this.btnSalvarMulta.Location = new System.Drawing.Point(486, 60);
			this.btnSalvarMulta.Name = "btnSalvarMulta";
			this.btnSalvarMulta.Size = new System.Drawing.Size(98, 23);
			this.btnSalvarMulta.TabIndex = 20;
			this.btnSalvarMulta.Text = "Gravar Multa";
			this.btnSalvarMulta.UseVisualStyleBackColor = true;
			this.btnSalvarMulta.Click += new System.EventHandler(this.BtnSalvarMulta_Click);
			// 
			// label4
			// 
			this.label4.AutoSize = true;
			this.label4.Location = new System.Drawing.Point(259, 48);
			this.label4.Name = "label4";
			this.label4.Size = new System.Drawing.Size(74, 13);
			this.label4.TabIndex = 23;
			this.label4.Text = "Nr. de pontos:";
			// 
			// txtPontosMulta
			// 
			this.txtPontosMulta.Location = new System.Drawing.Point(262, 64);
			this.txtPontosMulta.MaxLength = 2;
			this.txtPontosMulta.Name = "txtPontosMulta";
			this.txtPontosMulta.Size = new System.Drawing.Size(69, 20);
			this.txtPontosMulta.TabIndex = 18;
			this.txtPontosMulta.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtPontosMulta_KeyPress);
			// 
			// label29
			// 
			this.label29.AutoSize = true;
			this.label29.Location = new System.Drawing.Point(133, 48);
			this.label29.Name = "label29";
			this.label29.Size = new System.Drawing.Size(34, 13);
			this.label29.TabIndex = 21;
			this.label29.Text = "Valor:";
			// 
			// txtValorMulta
			// 
			this.txtValorMulta.Location = new System.Drawing.Point(136, 64);
			this.txtValorMulta.MaxLength = 10;
			this.txtValorMulta.Name = "txtValorMulta";
			this.txtValorMulta.Size = new System.Drawing.Size(100, 20);
			this.txtValorMulta.TabIndex = 16;
			this.txtValorMulta.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.txtValorMulta_KeyPress);
			// 
			// txtDescricaoMulta
			// 
			this.txtDescricaoMulta.Location = new System.Drawing.Point(136, 25);
			this.txtDescricaoMulta.Name = "txtDescricaoMulta";
			this.txtDescricaoMulta.Size = new System.Drawing.Size(602, 20);
			this.txtDescricaoMulta.TabIndex = 15;
			// 
			// label30
			// 
			this.label30.AutoSize = true;
			this.label30.Location = new System.Drawing.Point(133, 9);
			this.label30.Name = "label30";
			this.label30.Size = new System.Drawing.Size(101, 13);
			this.label30.TabIndex = 17;
			this.label30.Text = "Descrição da multa:";
			// 
			// label31
			// 
			this.label31.AutoSize = true;
			this.label31.Location = new System.Drawing.Point(10, 9);
			this.label31.Name = "label31";
			this.label31.Size = new System.Drawing.Size(76, 13);
			this.label31.TabIndex = 14;
			this.label31.Text = "Data da multa:";
			// 
			// comboPlaca
			// 
			this.comboPlaca.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			this.comboPlaca.FormattingEnabled = true;
			this.comboPlaca.Location = new System.Drawing.Point(73, 29);
			this.comboPlaca.Name = "comboPlaca";
			this.comboPlaca.Size = new System.Drawing.Size(121, 21);
			this.comboPlaca.TabIndex = 19;
			this.comboPlaca.SelectedIndexChanged += new System.EventHandler(this.ComboPlaca_SelectedIndexChanged);
			// 
			// txtNomeCarro
			// 
			this.txtNomeCarro.Location = new System.Drawing.Point(272, 29);
			this.txtNomeCarro.Name = "txtNomeCarro";
			this.txtNomeCarro.ReadOnly = true;
			this.txtNomeCarro.Size = new System.Drawing.Size(361, 20);
			this.txtNomeCarro.TabIndex = 20;
			// 
			// label1
			// 
			this.label1.AutoSize = true;
			this.label1.Location = new System.Drawing.Point(73, 10);
			this.label1.Name = "label1";
			this.label1.Size = new System.Drawing.Size(104, 13);
			this.label1.TabIndex = 21;
			this.label1.Text = "Selecione o Veiculo:";
			// 
			// FormDespesasCarro
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.ClientSize = new System.Drawing.Size(800, 450);
			this.Controls.Add(this.label1);
			this.Controls.Add(this.txtNomeCarro);
			this.Controls.Add(this.comboPlaca);
			this.Controls.Add(this.TabCadastroVeiculos);
			this.Name = "FormDespesasCarro";
			this.ShowIcon = false;
			this.Text = "Despesas do Veiculo";
			this.Load += new System.EventHandler(this.FormDespesasCarro_Load);
			this.TabCadastroVeiculos.ResumeLayout(false);
			this.tabAbastecimentos.ResumeLayout(false);
			this.groupBox3.ResumeLayout(false);
			this.groupBox3.PerformLayout();
			this.groupBox2.ResumeLayout(false);
			((System.ComponentModel.ISupportInitialize)(this.dataAbastecimento)).EndInit();
			this.groupAbastecimento.ResumeLayout(false);
			this.groupAbastecimento.PerformLayout();
			this.tabManutencao.ResumeLayout(false);
			this.groupBox5.ResumeLayout(false);
			this.groupBox5.PerformLayout();
			this.groupBox6.ResumeLayout(false);
			((System.ComponentModel.ISupportInitialize)(this.dataGridManutencao)).EndInit();
			this.groupManutencao.ResumeLayout(false);
			this.groupManutencao.PerformLayout();
			this.tabSinistros.ResumeLayout(false);
			this.groupBox7.ResumeLayout(false);
			this.groupBox7.PerformLayout();
			this.groupBox8.ResumeLayout(false);
			((System.ComponentModel.ISupportInitialize)(this.dataGridSinistros)).EndInit();
			this.groupSinistros.ResumeLayout(false);
			this.groupSinistros.PerformLayout();
			this.tabObrigacoes.ResumeLayout(false);
			this.tabObrigacoes.PerformLayout();
			this.groupBox10.ResumeLayout(false);
			((System.ComponentModel.ISupportInitialize)(this.dataGridObrigacoes)).EndInit();
			this.tabMultas.ResumeLayout(false);
			this.tabMultas.PerformLayout();
			((System.ComponentModel.ISupportInitialize)(this.dataGridMultas)).EndInit();
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		public System.Windows.Forms.TabControl TabCadastroVeiculos;
		private System.Windows.Forms.TabPage tabAbastecimentos;
		private System.Windows.Forms.GroupBox groupBox3;
		private System.Windows.Forms.Button button2;
		private System.Windows.Forms.Label label11;
		private System.Windows.Forms.Label label10;
		private System.Windows.Forms.DateTimePicker dateTimePicker2;
		private System.Windows.Forms.DateTimePicker dateTimePicker1;
		private System.Windows.Forms.GroupBox groupBox2;
		private System.Windows.Forms.DataGridView dataAbastecimento;
		private System.Windows.Forms.GroupBox groupAbastecimento;
		private System.Windows.Forms.Button btnAddAbastecimento;
		private System.Windows.Forms.TextBox txtTotalAbastecimento;
		private System.Windows.Forms.Label lblTotal;
		private System.Windows.Forms.TextBox txtValorUnitAbastecimento;
		private System.Windows.Forms.Label label6;
		private System.Windows.Forms.Label label9;
		private System.Windows.Forms.ComboBox comboCombustivelAbastecimento;
		private System.Windows.Forms.TextBox txtQtdLitrosAbastecimento;
		private System.Windows.Forms.Label label7;
		private System.Windows.Forms.Label label8;
		private System.Windows.Forms.TabPage tabManutencao;
		private System.Windows.Forms.GroupBox groupBox5;
		private System.Windows.Forms.Button button4;
		private System.Windows.Forms.Label label13;
		private System.Windows.Forms.Label label17;
		private System.Windows.Forms.DateTimePicker dateTimePicker3;
		private System.Windows.Forms.DateTimePicker dateTimePicker4;
		private System.Windows.Forms.GroupBox groupBox6;
		private System.Windows.Forms.DataGridView dataGridManutencao;
		private System.Windows.Forms.GroupBox groupManutencao;
		private System.Windows.Forms.RichTextBox txtDescricaoManutencao;
		private System.Windows.Forms.Button btnAddManutencao;
		private System.Windows.Forms.Label label12;
		private System.Windows.Forms.TextBox txtKmManutencao;
		private System.Windows.Forms.Label label14;
		private System.Windows.Forms.Label label15;
		private System.Windows.Forms.Label label16;
		private System.Windows.Forms.TabPage tabSinistros;
		private System.Windows.Forms.GroupBox groupBox7;
		private System.Windows.Forms.Button button5;
		private System.Windows.Forms.Label label18;
		private System.Windows.Forms.Label label19;
		private System.Windows.Forms.DateTimePicker dateTimePicker5;
		private System.Windows.Forms.DateTimePicker dateTimePicker6;
		private System.Windows.Forms.GroupBox groupBox8;
		private System.Windows.Forms.DataGridView dataGridSinistros;
		private System.Windows.Forms.GroupBox groupSinistros;
		private System.Windows.Forms.RichTextBox txtDescricaoSinistros;
		private System.Windows.Forms.Button btnAddSinistros;
		private System.Windows.Forms.Label label20;
		private System.Windows.Forms.TextBox txtKmSinistro;
		private System.Windows.Forms.TextBox txtValorSinistro;
		private System.Windows.Forms.Label label21;
		private System.Windows.Forms.Label label22;
		private System.Windows.Forms.Label label23;
		private System.Windows.Forms.TabPage tabObrigacoes;
		private System.Windows.Forms.Label lblValorTotalObrigacoes;
		private System.Windows.Forms.Label label28;
		private System.Windows.Forms.GroupBox groupBox10;
		private System.Windows.Forms.DataGridView dataGridObrigacoes;
		private System.Windows.Forms.Label label27;
		private System.Windows.Forms.Button btnAddObrigacoes;
		private System.Windows.Forms.TextBox txtValorObrigacoes;
		private System.Windows.Forms.Label label26;
		private System.Windows.Forms.TextBox txtDescricaoObrigacoes;
		private System.Windows.Forms.Label label25;
		private System.Windows.Forms.Label label24;
		private System.Windows.Forms.ComboBox comboPlaca;
		private System.Windows.Forms.TextBox txtNomeCarro;
		private System.Windows.Forms.Label label1;
		private System.Windows.Forms.TabPage tabMultas;
		private System.Windows.Forms.Label label2;
		private System.Windows.Forms.DataGridView dataGridMultas;
		private System.Windows.Forms.Button btnSalvarMulta;
		private System.Windows.Forms.Label label4;
		private System.Windows.Forms.TextBox txtPontosMulta;
		private System.Windows.Forms.Label label29;
		private System.Windows.Forms.TextBox txtValorMulta;
		private System.Windows.Forms.TextBox txtDescricaoMulta;
		private System.Windows.Forms.Label label30;
		private System.Windows.Forms.Label label31;
		private System.Windows.Forms.DateTimePicker dateMultaVencimento;
		private System.Windows.Forms.DateTimePicker dateMulta;
		private System.Windows.Forms.DateTimePicker dateAbastecimento;
		private System.Windows.Forms.DateTimePicker dateManutencao;
		private System.Windows.Forms.DateTimePicker dateSinistros;
		private System.Windows.Forms.DateTimePicker dateObrigacoes;
		private System.Windows.Forms.TextBox txtValorManutencao;
		private System.Windows.Forms.TextBox txtTipoObrigacao;
	}
}